...
class Krea2ControlInputProjection(nn.Module):
    ...
    def forward(self, image_tokens):
        ...
        if control_tokens.shape[1] != image_tokens.shape[1]:
            # A different-length token sequence is a NON-TARGET branch passing
            # through the shared input projection - the Identity Edit recipe
            # runs the reference photo's tokens (its own aspect, its own count)
            # through `first` too. Control applies only to the target-frame
            # branch (matching count); other branches project untouched.
            original_first = self.original_first
            if original_first is not None:
                return original_first(image_tokens)
            raise RuntimeError(
                f"Krea2 control token count mismatch with no original projection: "
                f"image={image_tokens.shape[1]}, control={control_tokens.shape[1]}."
            )
...
# ---------------------------------------------------------------------------
# Ostris edit reference conditioning - faithful port of
# ostris/ComfyUI-Krea2-Ostris-Edit nodes.py (MIT License, (c) 2026 Ostris,
# LLC), brought in-pack so the Ostris-trained Krea2 edit/control LoRAs (e.g.
# thedeoxen's krea2_turbo_openpose_controlnet) run without the external
# package. Mechanism: reference images are (a) vision-tokenized into the
# Qwen3-VL prompt via "Picture N:" placeholders under Krea's own template and
# (b) VAE-encoded and attached as `reference_latents`; the model patch then
# appends those latents to the image token sequence at RoPE axis-0 index
# 1,2,3... conditioned at t=0 ("index_timestep_zero"). The kv_cache toggle
# precomputes the refs' per-block attention K/V in one t=0 ref-only pass and
# injects them as extra keys every step (for LoRAs trained with ai-toolkit's
# kv_cache kwarg, where ref tokens attend only to each other).
# ---------------------------------------------------------------------------

_OSTRIS_VLM_MAX_PIXELS = 384 * 384       # Qwen3-VL sees a coarse reference
_OSTRIS_REF_LATENT_MAX_PIXELS = 1024 * 1024  # VAE ref latents carry the detail
_OSTRIS_REF_SNAP = 16                    # VAE f8 * patch 2 -> grid patchifies


def _ostris_fit_area(samples, max_pixels, snap=1):
...
def _ostris_pack_refs(dit, ref_latents, bs, device, dtype):
    ...
    for i, ref in enumerate(ref_latents):
        ...
        ref_tokens.append(
            rearrange(ref, "b c (h ph) (w pw) -> b (h w) (c ph pw)",
                      ph=patch, pw=patch))
...
def _ostris_attn_kv(attn, x, freqs, mask=None, kv_capture=None, kv_cache=None,
                    transformer_options={}):
...
def _ostris_block_kv_forward(block, x, vec, freqs, kv_capture=None,
                             kv_cache=None, transformer_options={}):
...
def _ostris_block_ref_forward(block, x, vec, refvec, split, freqs,
                              transformer_options):
    """SingleStreamBlock forward with per-span modulation: tokens [:split]
    (text + noisy image) use vec (real t), tokens [split:] (clean reference
    tokens) use refvec (t=0)."""
    m = block.mod(vec)
    r = block.mod(refvec)
    ...
    def mod(h, scale, shift):
        return torch.cat(
            ((1 + m[scale]) * h[:, :split] + m[shift],
             (1 + r[scale]) * h[:, split:] + r[shift]), dim=1)

    def gate(h, g):
        return torch.cat((m[g] * h[:, :split], r[g] * h[:, split:]), dim=1)

    x = x + gate(
        block.attn(mod(block.prenorm(x), 0, 1), freqs, None,
                   transformer_options=transformer_options), 2)
    x = x + gate(block.mlp(mod(block.postnorm(x), 3, 4)), 5)
    return x


def _ostris_forward_with_refs(dit, x, timesteps, context, ref_latents,
                              transformer_options):
    """Krea 2 SingleStreamDiT forward with reference latents appended at t=0
    ("index_timestep_zero"). Mirrors comfy.ldm.krea2.model.SingleStreamDiT.
    _forward (including the 5D Wan21-latent handling) with the ref packing /
    ...
    reftok, refpos = _ostris_pack_refs(dit, ref_latents, bs, device, x.dtype)
    reflen = reftok.shape[1]

    img = dit.first(torch.cat((img, reftok), dim=1))

    t = dit.tmlp(timestep_embedding(timesteps, dit.tdim).unsqueeze(1).to(img.dtype))
    tvec = dit.tproj(t)
    t0 = dit.tmlp(timestep_embedding(torch.zeros_like(timesteps), dit.tdim)
                  .unsqueeze(1).to(img.dtype))
    tvec0 = dit.tproj(t0)

    context = dit.txtfusion(context, mask=None,
                            transformer_options=transformer_options)
    ...
    for block in dit.blocks:
        combined = _ostris_block_ref_forward(
    ...
    out = rearrange(out, "b (h w) (c ph pw) -> b c (h ph) (w pw)",
                    h=h_, w=w_, ph=patch, pw=patch, c=dit.channels)
    out = out[:, :, :H_orig, :W_orig]
    if temporal:
...
def _ostris_precompute_ref_kv(dit, x, timesteps, ref_latents,
                              transformer_options):
    """Run only the clean reference tokens through the blocks at t=0 and
    record each block's post-RoPE K/V. An isolate-ref-attention LoRA never
    lets ref tokens see the text / noisy tokens, so this standalone pass
    produces exactly the K/V the refs would contribute inside a joint pass -
    and they are timestep-invariant, so one pass serves the whole denoise."""
    bs = x.shape[0] * (x.shape[2] if x.ndim == 5 else 1)
    reftok, refpos = _ostris_pack_refs(dit, ref_latents, bs, x.device, x.dtype)
    h = dit.first(reftok)
    t0 = dit.tmlp(timestep_embedding(torch.zeros_like(timesteps), dit.tdim)
...
def _ostris_forward_with_cached_refs(dit, x, timesteps, context, ref_kv,
                                     transformer_options):
    ...
    img = dit.first(rearrange(x, "b c (h ph) (w pw) -> b (h w) (c ph pw)",
                              ph=patch, pw=patch))
    ...
    for block, kv in zip(dit.blocks, ref_kv):
        combined = _ostris_block_kv_forward(
...
def _ostris_ref_fingerprint(ref_latents, bs):
...
class Krea2OstrisEditTextEncode:
    """Prompt + up to three reference images for an Ostris-trained Krea 2
    edit/control LoRA. Each image is fed to Qwen3-VL as a "Picture N:" vision
    token (coarse, 384x384 area cap) and, when `vae` is connected, VAE-encoded
    and attached to the conditioning as `reference_latents` (1MP cap, /16
    snap) for Krea2OstrisEditModelPatch to inject. Pose/control LoRAs from
    this ecosystem take the control map as image1; identity references ride
    as image2/image3."""

    CATEGORY = KREA2_CATEGORY
    TITLE = "Krea2 Ostris Edit Text Encode ⚡"
    SEARCH_ALIASES = ['ostris', 'edit encode', 'reference encode', 'pose control',
                      'kontext', 'picture']
    RETURN_TYPES = ("CONDITIONING",)
    FUNCTION = "encode"
    DESCRIPTION = ("Encode a prompt with optional reference images for an "
                   "Ostris-trained Krea 2 edit/control LoRA (vision tokens + "
                   "reference_latents). Pair with Krea2 Ostris Edit Model "
                   "Patch. Port of ostris/ComfyUI-Krea2-Ostris-Edit (MIT).")
    ...
    def encode(self, clip, prompt, vae=None, image1=None, image2=None, image3=None):
        ...
        for image in (image1, image2, image3):
            ...
            images_vl.append(
                _ostris_fit_area(samples, _OSTRIS_VLM_MAX_PIXELS).movedim(1, -1))
            if vae is not None:
                s = _ostris_fit_area(samples, _OSTRIS_REF_LATENT_MAX_PIXELS,
                                     snap=_OSTRIS_REF_SNAP)
                ref_latents.append(vae.encode(s.movedim(1, -1)[:, :, :, :3]))
            image_prompt += (
                "Picture {}: <|vision_start|><|image_pad|><|vision_end|>".format(
                    len(images_vl)))

        # Krea's own template (not the Qwen-Edit one): the trained LoRA saw
        # the base T2I system prompt with the Picture markers in the user
        # section.
...
class Krea2OstrisEditModelPatch:
    """Enable `reference_latents` on a Krea 2 model - the ai-toolkit
    "index_timestep_zero" method: reference tokens appended to the image
    sequence at RoPE axis-0 index 1,2,3..., conditioned at t=0. `kv_cache`
    switches to the cached one-pass reference mode for LoRAs trained with
    ai-toolkit's kv_cache kwarg (thedeoxen's pose ControlNet LoRA is one)."""

    CATEGORY = KREA2_CATEGORY
    TITLE = "Krea2 Ostris Edit Model Patch ⚡"
    SEARCH_ALIASES = ['ostris', 'edit patch', 'reference latents', 'kv cache',
                      'pose control']
    RETURN_TYPES = ("MODEL",)
    FUNCTION = "patch"
    DESCRIPTION = ("Reference-latent injection for Ostris-trained Krea 2 "
                   "edit/control LoRAs (index_timestep_zero; optional cached "
                   "ref K/V). Port of ostris/ComfyUI-Krea2-Ostris-Edit (MIT).")
        ...
        return {
            "required": {
                ...
                "kv_cache": ("BOOLEAN", {
                    "default": False,
                    "tooltip": (
                        "Cache the reference tokens' attention K/V: "
                        "precomputed in one t=0 pass and reused every "
                        "denoising step, so the refs never ride along in the "
                        "per-step sequence. The LoRA must be trained with "
                        "ai-toolkit's kv_cache model kwarg for this to work "
                        "properly. Leave off for normal edit LoRAs.")}),
            }
        }
    ...
    def patch(self, model, kv_cache=False):
        ...
        # pass per run (t=0, refs attend only to each other, matching
        # training) and reused by every denoising call. Cleared at the start
        # of each sampling run so LoRA/weight changes between runs never
        # serve stale K/V; keyed on ref content + batch size so cond/uncond
        # with different refs coexist.
        state = {"last_sigma": None, "caches": {}}

        def forward(x, timesteps, context, attention_mask=None,
                    transformer_options={}, ref_latents=None, **kwargs):
            if ref_latents is None or len(ref_latents) == 0:
                return orig_forward(
                    x, timesteps, context, attention_mask=attention_mask,
                    transformer_options=transformer_options, **kwargs)
            if not kv_cache:
                return _ostris_forward_with_refs(
                    dit, x, timesteps, context, ref_latents,
                    transformer_options)
            ...
            bs = x.shape[0] * (x.shape[2] if x.ndim == 5 else 1)
            key = _ostris_ref_fingerprint(ref_latents, bs)
            ref_kv = state["caches"].get(key)
            if ref_kv is None:
                ref_kv = _ostris_precompute_ref_kv(
                    dit, x, timesteps, ref_latents, transformer_options)
                state["caches"][key] = ref_kv
            return _ostris_forward_with_cached_refs(
...
NODE_CLASS_MAPPINGS = {
    ...
    # CCTech-prefixed ids (see the collision note above) - the classes are
    # ports of ostris/ComfyUI-Krea2-Ostris-Edit's two nodes.
    "CCTechKrea2OstrisEditTextEncode": Krea2OstrisEditTextEncode,
    "CCTechKrea2OstrisEditModelPatch": Krea2OstrisEditModelPatch,
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "CCTechKrea2OstrisEditTextEncode": Krea2OstrisEditTextEncode.TITLE,
    "CCTechKrea2OstrisEditModelPatch": Krea2OstrisEditModelPatch.TITLE,
