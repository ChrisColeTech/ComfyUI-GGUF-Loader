...
**Not every "Krea2 canny LoRA" is a Control LoRA.** There are two unrelated families of Krea2 LoRA in the wild, and only one of them goes through `Krea2 Control LoRA Loader`:

- **Widened-projection Control LoRAs** (e.g. the depth one) — ships with the DiT's `first` layer expanded, loaded via `Krea2 Control LoRA Loader` as described above, driven by `control_mode`/`control_image` on `Krea2 img2img`.
- **In-context / edit-style LoRAs** (e.g. [nynxz/NK2E](https://huggingface.co/nynxz/NK2E)'s `krea2_canny-v0.1.safetensors`) — an *ordinary* rank-32 LoRA with no widened projection at all (confirmed by inspecting its tensor keys: plain `lora_down`/`lora_up`/`alpha`, nothing else) — load it with **stock `LoraLoaderModelOnly`**, not `Krea2 Control LoRA Loader` (which will correctly reject it — there's no expanded weight to find). These work like Qwen-Image-Edit: "structure comes from the edges, content from the text prompt," via `Krea2 img2img`'s **`edit_reference`** input (below), not `control_image`.
...
Minimal graph for the depth LoRA: `Load Image` → `Krea2 img2img`'s `image` (that's it — depth derives automatically), alongside `Krea2 Model Loader` → `Krea2 Control LoRA Loader` → `Krea2 img2img` (prompt typed directly into this node) → stock `KSampler` → `VAE Decode`. For an in-context LoRA like the canny one: `Krea2 Model Loader` → stock `LoraLoaderModelOnly` (not `Krea2 Control LoRA Loader`) → `Krea2 img2img`'s `model`, with your canny map into `edit_reference`.

`tools/smoke_krea2.py` covers the tensor-prep helpers (grayscale/normalize/invert/resize), the `Krea2ControlInputProjection` forward math (image-only fallback, and image+control summation), `Krea2 img2img`'s guard rails, auto_depth/auto_canny derivation, manual-override precedence, and `edit_reference` attaching `reference_latents` to positive conditioning only (24/24, no GPU). Loading real GGUF/LoRA weights end-to-end (both the widened-projection depth LoRA and the plain canny LoRA via `LoraLoaderModelOnly`), the Depth Anything V2 port's `load_state_dict(strict=True)` against the real HuggingFace checkpoint, and `ref_latents` existing on the real loaded model's forward signature, were all verified separately against the actual portable ComfyUI environment.
...
**Qwen-Image Canny** is the standalone version of `control_mode="auto_canny"` (below) — plain `cv2.Canny` edge detection, no model, no download. Same role `Krea2 Depth Map` plays for depth: wire it in explicitly to preview the edge map before it goes into `control_image`, or reuse it elsewhere, instead of it happening invisibly inside `Qwen-Image img2img`.
...
`tools/smoke_qwen_image.py` covers the control-dispatch helper (conditioning stamping, hint/strength passthrough), `Qwen-Image img2img`'s guard rails, latent shapes, both control attachment paths (model_patch → cloned `MODEL`; controlnet → `CONDITIONING`, `MODEL` left untouched), `Qwen-Image Canny`'s output shape, and `edit_reference` attaching `reference_latents` to positive conditioning only — 10/10, no GPU. The format auto-detection and both control attachment mechanisms were verified separately against the real files (3 DiffSynth patches + the InstantX/Union checkpoint) in the actual portable ComfyUI environment.
