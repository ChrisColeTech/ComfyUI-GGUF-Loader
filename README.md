...
**LTX-2.3 ID-LoRA Prompt Editor** turns a captioner's raw two-part output (e.g. `LMStudioVisionPrompt`'s spoken-script-then-`---`-then-tagged-block text) into three editable fields — `visual_override`/`speech_override`/`sounds_override` — plus four `STRING` outputs: the reassembled `tagged_prompt` (feed straight into `CLIPTextEncode`) and the three fields individually (`speech_text` in particular is meant to feed a TTS node's `text` input directly, so editing `[SPEECH]` here and what actually gets spoken never drift apart). A paired `web/` JS extension auto-fills the three widgets with the parsed text right after the node runs — but only into a widget that's still empty, so an edit you typed is never clobbered by a later run. The parser (`_parse_id_lora_prompt`) stops each field at the next `[TAG]:` marker rather than running to end-of-string, so a middle section like `[SPEECH]` can't swallow `[SOUNDS]` the way a naive greedy regex would.
