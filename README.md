...
**Flux Klein Depth Map** is that same Depth Anything V2 detector exposed as its own standalone node — image in, depth map out — for wiring a depth reference into `Flux2 Klein Multi Reference Latent` (3+ references) or anywhere else a Klein reference image is wanted, mirroring `Krea2 Depth Map`'s existing explicit-node convention rather than hiding the preprocessing.
