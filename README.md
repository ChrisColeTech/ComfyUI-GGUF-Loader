...
**Flux Klein img2img** is the one-node prep step: `model, clip, vae, prompt, negative_prompt, strength, batch_size, width, height`, plus optional `image`, `reference_image`, `reference_image_2`, `control_mode`, `depth_ckpt_name`. The empty txt2img latent uses Flux.2's real shape (`[batch_size, 128, height // 16, width // 16]`, confirmed via comfy core's own `EmptyFlux2LatentImage`) — not the generic 4-channel/8-downscale placeholder used elsewhere in this pack, since comfy's `fix_empty_latent_channels` only auto-corrects channel count, not the spatial downscale ratio, unless `downscale_ratio_spacial` is explicitly passed.

`width`/`height` are a **pixel budget**, not an exact literal size, confirmed by tracing the actual node graph inside the real shipped example workflow (`image_flux2_klein_image_edit_9b_base.json`): whenever `image`, `reference_image`, or `reference_image_2` is connected, the real canvas is re-derived from that photo's own aspect ratio at the same total-pixel budget (`_scale_to_megapixels`, an aspect-preserving port of comfy core's own `ImageScaleToTotalPixels` math, rounded to the nearest multiple of 16 to stay aligned with Flux.2's `/16` latent downscale), with `image` taking priority over `reference_image` over `reference_image_2` for sizing purposes — exactly mirroring the example workflow's own `ImageScaleToTotalPixels` → `GetImageSize` → `EmptyFlux2LatentImage` chain, which never trusts an independent user-typed size when a photo is connected. This was a real bug before it was fixed: `width`/`height` used to be trusted as exact regardless of any connected photo, silently distorting the reference image via a center-crop resize to a mismatched aspect ratio and generating a canvas that didn't match it. Pure txt2img (no photos at all) still uses `width`/`height` literally.

`reference_image` is Klein's real editing mechanism, confirmed by tracing the actual node graph inside the shipped example workflows rather than assumed: it starts from a *pure-noise* `EmptyFlux2LatentImage` — **not** an img2img partial denoise of the edited photo — and drives the whole edit off reference images VAE-encoded and attached to positive **and** negative conditioning as `reference_latents`, plus a text instruction. `image` (the img2img partial-denoise starting point) and `reference_image` (conditioning-only reference) are independent — they answer different questions, same convention as `edit_reference` on `Krea2Img2Img`/`Qwen-Image img2img`.

`reference_image_2` is an optional second reference image, matching the real dual-reference example subgraph (`image_flux2_klein_image_edit_9b_base.json`, node 92 — e.g. "apply the logo from image 2 onto the car in image 1"): it always attaches **raw** (no `control_mode` preprocessing — that only applies to `reference_image`), chained onto positive and negative conditioning *after* `reference_image`'s own attachment, exactly mirroring the real workflow's two sequential `ReferenceLatent` calls per conditioning branch. It works standalone too, without `reference_image` connected.

`control_mode` — `manual` (default) attaches `reference_image` raw; `auto_depth` runs it through this pack's Depth Anything V2 first, reproducing the example workflow's own trick of feeding one reference through `AIO_Preprocessor` (set to `MiDaS-DepthMapPreprocessor`) before encoding it — the one mode CONFIRMED meaningful to Klein's own training; `auto_canny` does the same with plain `cv2.Canny`, mechanically valid but unverified for Klein specifically; `none` skips reference attachment entirely even if `reference_image` is connected, for toggling it off without rewiring. This is the same minimum set `Krea2Img2Img`/`QwenImageImg2Img` use — for normal maps/soft edges/lines/pose, install [ComfyUI-ControlNet-Nodes](https://github.com/ChrisColeTech/ComfyUI-ControlNet-Nodes) and wire its output into `reference_image` yourself with `control_mode=manual`. `depth_ckpt_name` picks the Depth Anything V2 model size (downloads on first use, same as `Krea2 Depth Map`).
...
Graph: `Flux Klein Model Loader` → `Flux Klein img2img` (prompt typed directly into this node, `reference_image` for Klein's real edit mechanism) → optionally `Flux Klein Identity Feature Transfer` on `model` → stock `KSampler` → `VAE Decode`. For 3+ references, more control types, color/identity anchoring, or per-reference weighting, install ComfyUI-Flux-Reference-Tools and/or ComfyUI-ControlNet-Nodes and chain their nodes in — see each package's own README for graph examples.

`tools/smoke_flux_klein.py` covers all 6 nodes (`FluxKleinModelLoader`, `FluxKleinImg2Img`, `Flux2KleinDepthMap` alias, `Flux2KleinIdentityFeatureTransfer`, `Flux2KleinEnhancer`, `Flux2KleinSectionedEncoder`) — `Flux Klein img2img`'s latent shapes (real Flux.2 128-channel shape, denoise=strength for img2img, batch repeat), its `_scale_to_megapixels` canvas-sizing helper (aspect ratio preserved, hits the requested megapixel budget, rounds to 16), the canvas correctly deriving from a connected non-square `reference_image` instead of the square widget defaults, `image` taking sizing priority over `reference_image` when both are connected, `reference_image_2` attaching raw alongside `reference_image` (and standalone), its `reference_image`/`control_mode` attachment (manual attaches raw to both positive+negative, auto_depth/auto_canny each derive for real, `none` skips attachment even when connected, `control_mode`'s option list matches the minimum set exactly), `Flux Klein Depth Map`'s delegation to the shared depth helper, `Flux Klein Identity Feature Transfer`'s hook registration and similarity-pull math on a synthetic attention tensor, Enhancer's no-op passthrough and active-region scaling, and Sectioned Encoder's `klein_sections` emission with a fake HF tokenizer and its no-tokenizer fallback — 26/26, no GPU. Native Flux2 detection, `CLIPType.FLUX2`, the real `clip.tokenizer.qwen3_4b.tokenizer` attribute path, `control_mode=none`/`auto_canny` end-to-end, the canvas-sizing fix, and `reference_image_2`'s dual-attachment were all verified separately against the actual portable ComfyUI environment, the user's real `flux-2-klein-9b` GGUF checkpoint, a real loaded Klein CLIP object, and the real Flux.2 VAE.
