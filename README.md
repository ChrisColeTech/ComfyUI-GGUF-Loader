...
Graph: `Flux Klein Model Loader` → `Flux Klein img2img` (prompt typed directly into this node) → `Flux2 Klein Multi Reference Latent` (chain reference photos in) → optionally `Flux Klein Identity Feature Transfer` on `model` → stock `KSampler` → `VAE Decode`.

`tools/smoke_flux_klein.py` covers `Flux Klein img2img`'s latent shapes (real Flux.2 128-channel shape, denoise=strength for img2img, batch repeat), `Flux2 Klein Multi Reference Latent`'s batch-splitting/ordering/overwrite behavior and dual positive+negative attachment, and `Flux Klein Identity Feature Transfer`'s hook registration (enabled/disabled, focus_only vs. zero_unmasked_tokens+mask), schedule parsing, and the actual similarity-pull math on a synthetic attention tensor — 14/14, no GPU. Native Flux2 detection, `CLIPType.FLUX2`, node registration, and `set_model_attn1_patch`/`set_model_attn1_output_patch` being real `ModelPatcher` methods that accept a real GGUF-loaded Klein model were all verified separately against the actual portable ComfyUI environment and the user's real `flux-2-klein-9b` GGUF checkpoint.
