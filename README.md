...
`tools/smoke_flux_klein.py` covers all 6 nodes (`FluxKleinModelLoader`, `FluxKleinImg2Img`, `Flux2KleinDepthMap` alias, `Flux2KleinIdentityFeatureTransfer`, `Flux2KleinEnhancer`, `Flux2KleinSectionedEncoder`) — `Flux Klein img2img`'s latent shapes (real Flux.2 128-channel shape, denoise=strength for img2img, batch repeat), its `reference_image`/`control_mode` attachment (manual attaches raw to both positive+negative, auto_depth/auto_canny each derive for real, `none` skips attachment even when connected, `control_mode`'s option list matches the minimum set exactly), `Flux Klein Depth Map`'s delegation to the shared depth helper, `Flux Klein Identity Feature Transfer`'s hook registration and similarity-pull math on a synthetic attention tensor, Enhancer's no-op passthrough and active-region scaling, and Sectioned Encoder's `klein_sections` emission with a fake HF tokenizer and its no-tokenizer fallback — 20/20, no GPU. Native Flux2 detection, `CLIPType.FLUX2`, the real `clip.tokenizer.qwen3_4b.tokenizer` attribute path, `control_mode=none`/`auto_canny` end-to-end, and the exact 50-node total after this trim were all verified separately against the actual portable ComfyUI environment, the user's real `flux-2-klein-9b` GGUF checkpoint, a real loaded Klein CLIP object, and the real Flux.2 VAE.
